...
function UIFunctions.Init(G2L, window)
    ...
    G2L["6"].InputBegan:Connect(function(i)
    ...
    -- Close & Fullscreen
    G2L["72"].MouseButton1Click:Connect(function() G2L["1"]:Destroy() end)
    ...
    task.spawn(function()
        while task.wait(1) and G2L["1"].Parent do
            window.Stats.FPS.Text, G2L["time_text"].Text = math.floor(1/RunService.RenderStepped:Wait()), os.date("%I:%M %p")
            pcall(function()
